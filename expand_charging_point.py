...
class ChargePoint(CP):
    ...
    # remosteStartTransaction.conf(status)
    ...
    # remosteStopTransaction.conf(status)
    @on(Action.RemoteStartTransaction)
...
async def main():
    async with websockets.connect(
            ...
            uri='ws://localhost:8000/ws/CP_1312'
    ) as ws:
        cp = ChargePoint('CP_1312', ws)
        ...
        await console_task
        # await cp.start()
        # start=asyncio.gather(cp.start())
        # send_boot=asyncio.gather(cp.send_boot_notification())
        # send_start=asyncio.gather(cp.start_transaction_notification())
        # send_stop=asyncio.gather(cp.stop_transaction_notification())
        # send_meter=asyncio.gather(cp.send_meter_values())

        # await asyncio.gather(start,send_boot,send_start,send_stop,send_meter)
        # await asyncio.gather(cp.start(),cp.send_boot_notification(),cp.start_transaction_notification(), cp.stop_transaction_notification(),cp.send_meter_values())
